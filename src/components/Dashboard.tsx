...
const ChannelCard: React.FC<ChannelCardProps> = ({
  ...
  return <Card className={cn("cursor-pointer transition-all duration-200 hover:shadow-md border", isPinned ? "ring-2 ring-offset-2" : "")} style={{
    ...
    ...(isPinned && {
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          e.stopPropagation();
          onTogglePin(id);
        }} className={cn("h-6 w-6 p-0", isDarkMode ? "text-gray-400 hover:text-gray-300" : "text-gray-400 hover:text-gray-600")}>
            {isPinned ? '📌' : '📍'}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        <div className="flex items-center justify-between">
          <span className={cn("text-2xl font-bold", isDarkMode ? "text-gray-300" : "text-gray-700")}>
            {conversationCount}
          </span>
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  return <div className="space-y-4 md:space-y-6 min-h-screen p-3 md:p-6" style={{
    backgroundColor: isDarkMode ? '#000000' : '#f9fafb'
      ...
      {/* Exam Chart Section */}
      <div>
        <h2 className="text-lg md:text-xl font-semibold mb-4" style={{
        color: '#b5103c'
      }}>
