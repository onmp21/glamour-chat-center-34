...
export const ChannelsSection: React.FC<ChannelsSectionProps> = ({
  ...
  return <div>
      ...
      <div className="hidden sm:grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 md:gap-4 lg:gap-5">
        {availableChannels.map(channel => <NewChannelCard key={channel.id} name={channel.name} subtitle={channel.id === "chat" ? "Canal Geral" : `Cidade: ${channel.name}`} count={channel.conversationCount} isDarkMode={isDarkMode} status={"online"} onClick={() => onChannelClick(channel.id)} compact={true} />)}
      </div>
