...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const allChannelItems = [{
    ...
    id: 'gerente-lojas',
    label: 'Gerente das Lojas'
  }, {
    id: 'gerente-externo',
    label: 'Gerente do Externo'
  }];
  const accessibleChannels = getAccessibleChannels();
  const channelItems = allChannelItems.filter(channel => accessibleChannels.includes(channel.id));
  ...
  return <>
      ...
      <MobileSidebar activeSection={activeSection} onSectionChange={onSectionChange} isDarkMode={isDarkMode} toggleDarkMode={toggleDarkMode} isOpen={isMobileSidebarOpen} onClose={() => setIsMobileSidebarOpen(false)} />
      ...
      borderColor: isDarkMode ? '#686868' : '#e5e7eb'
        ...
        borderColor: isDarkMode ? '#686868' : '#e5e7eb'
          ...
          <div className="flex items-center space-x-3">
            <div className="flex items-center justify-center">
              <img src="/lovable-uploads/ea397861-5fcd-451b-872e-727208c03a67.png" alt="Villa Glamour Logo" className="w-16 h-16 object-contain app-logo" />
            </div>
        ...
        <nav className="flex-1 p-3 space-y-1">
          ...
          return <button key={item.id} onClick={() => onSectionChange(item.id)} className={cn("w-full flex items-center space-x-3 px-3 py-2 rounded-md text-left transition-colors text-sm", activeSection === item.id ? "text-white" : isDarkMode ? "text-white" : "text-gray-700")} style={{
          ...
          {/* Canais section */}
          <div>
            <button onClick={() => setIsChannelsExpanded(!isChannelsExpanded)} className={cn("w-full flex items-center justify-between px-3 py-2 rounded-md text-left transition-colors text-sm", isDarkMode ? "text-white" : "text-gray-700")} onMouseEnter={e => {
            e.currentTarget.style.backgroundColor = isDarkMode ? '#686868' : '#f3f4f6';
          }} onMouseLeave={e => {
            e.currentTarget.style.backgroundColor = 'transparent';
          }}>
            ...
            {isChannelsExpanded && <div className="ml-6 mt-1 space-y-1">
                {channelItems.map(channel => <button key={channel.id} onClick={() => onSectionChange(channel.id)} className={cn("w-full flex items-center px-3 py-1.5 rounded-md text-left transition-colors text-sm", activeSection === channel.id ? "text-white" : isDarkMode ? "text-white" : "text-gray-600")} style={{
