...
export class MessageFormatDetector {
  static detect(messageJson: any): FormatDetectionResult {
    ...
    // LANGCHAIN_OBJECT: Objeto complexo com additional_kwargs, response_metadata
    ...
    // LANGCHAIN_STRING: String JSON que contém estrutura LangChain
    if (typeof messageJson === 'string' && data && typeof data === 'object' &&
        (data.additional_kwargs !== undefined || data.response_metadata !== undefined)) {
      console.log('📄 LANGCHAIN_STRING detectado');
      return { format: MessageFormat.LANGCHAIN_STRING, confidence: 0.9, rawData: data };
    }
    ...
    // SIMPLE_JSON: Formato simples com type e content
    if (data && typeof data === 'object' && data.type && data.content !== undefined) {
      console.log('📝 SIMPLE_JSON detectado');
      return { format: MessageFormat.SIMPLE_JSON, confidence: 0.8, rawData: data };
    }
